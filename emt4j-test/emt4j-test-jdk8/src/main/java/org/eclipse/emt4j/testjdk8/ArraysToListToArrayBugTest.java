...
public class ArraysToListToArrayBugTest extends SITBaseCase {
    public void run() {
        ...
        try {
            uriArray[0] = new URI("http://www.bar1.com");
            uriArray[1] = new URI("http://www.bar2.com");
            uriArray[2] = new URI("http://www.bar3.com");
            List<URI> uriList = Arrays.asList(uriArray);
            Collections.shuffle(uriList);
            URI[] metastoreUris = (URI[]) uriList.toArray();
    ...
    public void verify(JsonReport jsonReport) {
        assertTrue(jsonReport.getResultDetailList().stream().filter((d) -> d.getMainResultCode().equals("ARRAYS_AS_LIST_TO_ARRAY"))
                .findAny().isPresent(), "Expect found ARRAYS_AS_LIST_TO_ARRAY,but actually not found");
    }
}
