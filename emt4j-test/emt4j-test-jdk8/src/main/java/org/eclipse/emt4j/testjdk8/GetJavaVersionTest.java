...
public class GetJavaVersionTest extends SITBaseCase {

    public void run() {
        String javaVersion = System.getProperty("java.version");
        getMajorVersion(javaVersion);
    }

    private String getMajorVersion(String javaVersion) {
        if (javaVersion.startsWith("1.")) {
            return javaVersion.substring(2, 3);
        } else {
            int dot = javaVersion.indexOf(".");
            if (dot != -1) {
                return javaVersion.substring(0, dot);
            }
            return javaVersion;
        }
    }
