...
@TestConf(mode = TestConf.ModeEnum.AGENT, from = TestConf.RELEASE.JDK17, to = TestConf.RELEASE.JDK21, option = "-XX:+G1UseAdaptiveConcRefinement")
public class JavaOptionTest extends SITBaseCase {
    ...
    public void verify(JsonReport jsonReport) {
        ...
        for (MainResultDetail mrd : list) {
            for (SubResultDetail srd : mrd.getSubResultDetailList()) {
                for (String solution : srd.getHowToFix()) {
                    if (solution.indexOf("-XX:+G1UseAdaptiveConcRefinement") != -1) {
        ...
        assertTrue(checkSuggestionOK, "Solution should contain -XX:+G1UseAdaptiveConcRefinement,but actually not found!");
    }
}
